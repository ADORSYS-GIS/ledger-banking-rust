...
use banking_api::service::{
    EntityReferenceService, EntityReferenceServiceError, EntityReferenceServiceResult,
};
...
fn map_domain_error_to_service_error(
    error: EntityReferenceRepositoryError,
) -> EntityReferenceServiceError {
    match error {
        ...
        } => EntityReferenceServiceError::DuplicateReference {
            person_id,
            reference_type,
            reference_external_id,
        },
        EntityReferenceRepositoryError::RepositoryError(err) => {
            EntityReferenceServiceError::RepositoryError(err)
        }
    }
}
...
#[async_trait]
impl<DB: Database + Send + Sync> EntityReferenceService for EntityReferenceServiceImpl<DB> {
    ...
    async fn find_entity_reference_by_id(
        &self,
        id: Uuid,
    ) -> EntityReferenceServiceResult<Option<EntityReference>> {
        let model_idx = self
            .repositories
            .entity_reference_repository
            .find_by_id(id)
